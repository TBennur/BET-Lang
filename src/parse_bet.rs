...
fn parse_bet_binding(lexpr: &Lexpr) -> Result<(String, Expr), String> {
    ...
    match &tokens[..] {
        [Atom(S(id)), Atom(S(walrus)), rest @ ..] if walrus == ":=" => {
            Ok((id_to_string(id), parse_bet_expr(&unwrap_lexpr_list(rest))))
        }
        _ => Err(format!("Not a valid binding: {:?}", lexpr)),
    }
}

fn parse_bet_expr(lexpr: &Lexpr) -> Expr {
    match lexpr {
        // atoms
        ...
        List(vec) => match &vec[..] {
                ...
                if if_kwd == "if" && else_kwd == "else" =>
            {
                ...
                Expr::If(
                    Box::new(parse_bet_expr(cond)),
                    Box::new(parse_bet_block(true_block).unwrap()),
                    Box::new(parse_bet_block(false_block).unwrap()),
                )
            }
            ...
            [Atom(S(keyword)), ParenList(bindings), scoped_block] if keyword == "let" => {
                ...
                Expr::Let(
                    bindings
                        .into_iter()
                        .map(|binding| parse_bet_binding(binding).unwrap())
                        .collect(),
                    Box::new(parse_bet_block(scoped_block).unwrap()),
                )
            }
            ...
            [Atom(S(op1)), rest @ ..] if UNOPS.contains(&op1.as_str()) => Expr::UnOp(
                ...
                Box::new(parse_bet_expr(&unwrap_lexpr_list(rest))),
            ),
            ...
            [Atom(S(fun_name)), ParenList(fun_args)] if !is_keyword(fun_name) => Expr::Call(
                ...
                fun_args.into_iter().map(parse_bet_expr).collect(),
            ),
            ...
            [lhs, Atom(S(op2)), rhs] if BET_BINOPS.contains(&op2.as_str()) => Expr::BinOp(
                ...
                Box::new(parse_bet_expr(lhs)),
                Box::new(parse_bet_expr(rhs)),
            ),
                ...
                if do_kwd == "do" && while_kwd == "until" =>
            {
                ...
                Expr::RepeatUntil(
                    Box::new(parse_bet_block(body_block).unwrap()),
                    Box::new(parse_bet_expr(cond)),
                )
            }
            ...
            [first @ .., Atom(S(dot)), Atom(S(field_name))] if dot == "." => Expr::Lookup(
                Box::new(parse_bet_expr(&unwrap_lexpr_list(first))),
        ...
        CurlyList(exprns) => {
            if exprns.len() == 0 {
                ...
                Expr::Block(
                    exprns
                        .into_iter()
                        .map(|element| parse_bet_expr(element))
                        .collect(),
                )
            }
        }
...
fn parse_bet_block(lexpr: &Lexpr) -> Result<Expr, String> {
    if let CurlyList(_stuff) = lexpr {
        Ok(parse_bet_expr(lexpr))
    } else {
        Err(format!(
...
fn parse_bet_struct_def(s: &Lexpr) -> Option<UserStruct> {
    let decl = match s {
...
fn parse_bet_fn_def(s: &Lexpr) -> Option<UserFunction> {
    ...
    Some(UserFunction::UserFun(
        ...
        parse_bet_block(fun_body).unwrap(),
    ))
}

pub fn parse_bet_program(lexpr: &Lexpr) -> Prog {
    ...
    for definition in definitions {
        if let Some(parsed) = parse_bet_fn_def(definition) {
            // must be a function
            fn_defs.push(parsed);
            continue;
        }

        if let Some(parsed) = parse_bet_struct_def(definition) {
            // must be a struct
